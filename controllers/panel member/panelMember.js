...
import StudentDocuments from '../../models/student/document.js'
...
const groupIDs = async (request, response, next) => {
    ...
    try {
        const groupIDs = await Group.find({ $or: [{ "panelMember1" : panelMemberId }, { "panelMember2" : panelMemberId }, { "panelMember3" : panelMemberId }] }).select("groupId -_id");
        var IDArray = []
...
const getStudentPresentations = async (request, response) => {
    ...
    try {
        ...
        return response.status(400).json({ error : error, message : "Unable to fetch presentations" });
    }
};
...
const getStudentPresentation = async (request, response) => {
    ...
    try {
        ...
        return response.status(400).json({ error : error, message : "Unable to the fetch presentation" });
    }
}
