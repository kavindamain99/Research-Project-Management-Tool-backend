...
const StudentDocument = require("../../models/student/document.js");
...
const getAllTopics = async (request, response) => {
  ...
  try {
    ...
    return response
      .status(400)
      .json({ message: "Field not found", error: error });
  }
};
...
const updateState = async (request, response) => {
  ...
  if (state === "accepted" || state === "rejected") {
    if (!evaluated) {
      ...
      return response
        .status(400)
        .json({ error: "Cannot change state after evaluation" });
    }
  } else {
    return response.status(400).json({ error: "Invalid state" });
  }
...
const groupIDs = async (request, response, next) => {
  try {
    const groupIDs = await Topic.find({
      supervisorId: request.supervisor.id,
    }).select("groupId -_id");
